...
public class AItemsets {
	 ...
	 */
	public AItemsets(String name) {
		this.name = name;
		levels.add(new ArrayList<AItemset>()); // We create an empty level 0 by
												// default.
	}
	...
	public void printItemsets(int nbObject) {
		...
		for (List<AItemset> level : levels) {
			// print how many items are contained in this level
			System.out.println("  L" + levelCount + " ");
			// for each itemset
			for (AItemset itemset : level) {
				// print the itemset
	 ...
	 */
	public void addItemset(AItemset itemset, int k) {
		while (levels.size() <= k) {
			levels.add(new ArrayList<AItemset>());
		}
	 ...
	 */
	public List<List<AItemset>> getLevels() {
		return levels;
	}
