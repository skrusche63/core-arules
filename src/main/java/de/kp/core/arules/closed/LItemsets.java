...
public class LItemsets {
	 ...
	 */
	public LItemsets(String name) {
		this.name = name;
		levels.add(new ArrayList<LItemset>()); // We create an empty level 0 by
												// default.
	}
	...
	public void printItemsets(int nbObject) {
		...
		for (List<LItemset> level : levels) {
			// print how many items are contained in this level
			System.out.println("  L" + levelCount + " ");
			// for each itemset
			for (LItemset itemset : level) {
				// print the itemset
	 ...
	 */
	public void addItemset(LItemset itemset, int k) {
		while (levels.size() <= k) {
			levels.add(new ArrayList<LItemset>());
		}
	 ...
	 */
	public List<List<LItemset>> getLevels() {
		return levels;
	}
